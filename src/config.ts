...
// Custom type to fix TypeScript error with Vite's import.meta.env
interface ImportMetaEnv {
  MODE: string;
  VITE_API_URL?: string;
  VITE_BASE_URL?: string;
  VITE_ENABLE_DATA_EXPORT?: string;
  VITE_ENABLE_DATA_IMPORT?: string;
  VITE_ENABLE_CHARTING?: string;
  VITE_ENABLE_REPORTING?: string;
...
const NODE_ENV = (import.meta.env as ImportMetaEnv).MODE || "development";
const API_URL = (import.meta.env as ImportMetaEnv).VITE_API_URL;
const BASE_URL = (import.meta.env as ImportMetaEnv).VITE_BASE_URL;
...
const config: Record<string, any> = {
  ...
  features: {
    ...
    enableDataExport: (import.meta.env as ImportMetaEnv).VITE_ENABLE_DATA_EXPORT === "true" || true, // Enable data export functionality
    enableDataImport: (import.meta.env as ImportMetaEnv).VITE_ENABLE_DATA_IMPORT === "true" || true, // Enable data import functionality
    enableCharting: (import.meta.env as ImportMetaEnv).VITE_ENABLE_CHARTING === "true" || true, // Enable charting and visualization features
    enableReporting: (import.meta.env as ImportMetaEnv).VITE_ENABLE_REPORTING === "true" || true, // Enable reporting features
    enableNotifications:
      (import.meta.env as ImportMetaEnv).VITE_ENABLE_NOTIFICATIONS === "true" || true, // Enable notifications
    enableOfflineMode:
      (import.meta.env as ImportMetaEnv).VITE_ENABLE_OFFLINE_MODE === "true" || true, // Enable offline functionality
    enableSubscriptions:
      (import.meta.env as ImportMetaEnv).VITE_ENABLE_SUBSCRIPTIONS === "true" || false, // Enable subscription features
  },
  ...
  ui: {
    ...
    layout: {
      sidebarWidth: "250px", // Sidebar width in pixels
      topbarHeight: "64px", // Top bar height in pixels
      contentMaxWidth: "1200px", // Content max width in pixels
      footerHeight: "60px", // Footer height in pixels
    },
    animation: {
      enabled: true,
      duration: 300, // Default animation duration in milliseconds
      easing: "cubic-bezier(0.4, 0, 0.2, 1)", // Default animation easing
    },
    ...
    responsive: {
      xs: 0,
      sm: 640, // Mobile breakpoint in pixels
      md: 768, // Tablet breakpoint in pixels
      lg: 1024, // Desktop breakpoint in pixels
      xl: 1280, // Large desktop breakpoint in pixels
    },
  },
  ...
  uploads: {
    ...
    allowedTypes: [".csv", ".xlsx", ".json", ".pdf"], // Allowed file types
