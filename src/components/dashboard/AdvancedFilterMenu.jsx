...
function FieldFilter({ field, fieldType, onChange, onRemove, value, index }) {
	...
	const renderValueInput = () => {
		...
		if (fieldType === COLUMN_TYPES.DATE) {
			return (
				<div className="flex items-center gap-2">
					<input type="date" className="h-8 px-2 py-1 text-sm border rounded focus:outline-none focus:ring-2 focus:ring-blue-500" value={filterValue} onChange={e => setFilterValue(e.target.value)} />
					{operator === "between" && (
						<>
							<span className="text-sm">and</span>
							<input
	...
	return (
		<div className="flex flex-col w-full p-2 mb-2 bg-white border rounded-md shadow-sm dark:bg-gray-800 dark:border-gray-700">
			<div className="flex items-center justify-between w-full mb-2">
				...
				<button onClick={onRemove} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" aria-label="Remove filter">
					<X size={16} />
				</button>
			</div>
			...
			<div className="flex flex-col gap-2">
				<div className="relative">
					<button onClick={() => setIsOpen(!isOpen)} className="flex items-center justify-between w-full h-8 px-2 text-sm text-left border rounded hover:bg-gray-50 dark:hover:bg-gray-700">
						<span>{operator}</span>
						<ChevronDown size={14} />
					</button>
...
export default function AdvancedFilterMenu({ isVisible, onClose }) {
	...
	// Get the available columns and their types
	useEffect(() => {
	...
	function determineColumnType(columnName) {
		...
		if (data && data.length > 0) {
			const sample = data.find(item => item[columnName] !== null && item[columnName] !== undefined);
			if (sample) {
	...
	const addFilter = () => {
		...
		const fieldType = availableColumns.find(c => c.name === selectedField)?.type || COLUMN_TYPES.STRING;
		...
		const filtered = data.filter(item => {
			return filters.every(filter => {
				...
				if (type === COLUMN_TYPES.NUMBER) {
					...
					if (operator === "between") return numItemValue >= numValue && numItemValue <= numSecondValue;
				}
				...
				if (type === COLUMN_TYPES.DATE) {
					...
					if (operator === "between") return dateItemValue >= dateValue && dateItemValue <= dateSecondValue;
				}
				...
				<div className="p-4 max-h-[70vh] overflow-y-auto">
					...
					<div className="flex items-center mb-4 space-x-2">
						<div className="flex-1">
							<select value={selectedField} onChange={e => setSelectedField(e.target.value)} className="w-full p-2 border border-gray-300 rounded-md dark:bg-gray-800 dark:border-gray-700">
					...
					{/* Active filters */}
					<div className="mb-4">
						<h3 className="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">Active Filters ({filters.length})</h3>

						{filters.length === 0 ? (
							<p className="p-3 text-center text-sm text-gray-500 bg-gray-50 rounded-md dark:bg-gray-800 dark:text-gray-400">No filters applied. Add a filter to start filtering your data.</p>
						) : (
							<div className="space-y-2">
								{filters.map((filter, index) => (
									<FieldFilter key={index} field={filter.field} fieldType={filter.type} onChange={handleFilterChange} onRemove={() => removeFilter(index)} value={filter} index={index} />
								))}
							</div>
						)}
					</div>
					...
					<div className="p-3 mb-4 border border-gray-200 rounded-md dark:border-gray-700">
						<h3 className="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">Save Current Filter</h3>
						<div className="flex items-center space-x-2">
							...
							/>
							<Button onClick={saveFilter} variant="secondary" disabled={!filterName || filters.length === 0} className="whitespace-nowrap">
					...
					{savedFilters.length > 0 && (
						<div className="mb-4">
							<h3 className="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">Saved Filters</h3>
									...
									<div key={index} className="flex items-center justify-between p-3">
										<div>
											...
											<div className="text-xs text-gray-500">
												{new Date(savedFilter.created).toLocaleDateString()} ·{savedFilter.filters.length} {savedFilter.filters.length === 1 ? "filter" : "filters"}
											</div>
										</div>
										<div className="flex items-center space-x-2">
											<button onClick={() => loadSavedFilter(savedFilter)} className="p-1 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300" title="Load filter">
												<RefreshCw size={16} />
											</button>
											<button onClick={() => removeSavedFilter(index)} className="p-1 text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300" title="Delete filter">
												<X size={16} />
											</button>
										</div>
									</div>
