...
export default function MobileDashboard({ financialData, projectData, countryData, onFilter, onSettings }) {
	const theme = useTheme();
	const { t } = useI18n();
	const [activeChart, setActiveChart] = useState("revenue-ebitda");
	const [menuOpen, setMenuOpen] = useState(false);
	const [favoriteCharts, setFavoriteCharts] = useState(["revenue-ebitda", "project-portfolio", "geographic-map", "cash-flow-sankey"]);
		...
		<Box>
			...
			<Box
				...
				<Tabs
					...
					{favoriteCharts.map(chartId => {
						...
						return <Tab key={chartId} value={chartId} icon={<Icon fontSize="small" />} iconPosition="start" label={chart.name} />;
					})}
				</Tabs>
			...
			<Box
				...
				<AnimatePresence mode="wait">
					<motion.div key={activeChart} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }} style={{ height: "100%" }}>
			...
			<SwipeableDrawer anchor="left" open={menuOpen} onClose={() => setMenuOpen(false)} onOpen={() => setMenuOpen(true)}>
