...
export default function DraggableDashboard({
  ...
  const forceGridUpdate = () => {
    ...
    setTimeout(() => {
      // Trigger resize event to reflow charts
      window.dispatchEvent(new Event("resize"));

      // Force a rerender by updating a layout clone
  ...
  const handleChangeCardSize = (chartId, size) => {
    ...
    if (chartLayout) {
      ...
      // Map size name to width percentage of container
      const sizeToPercent = {
        "1/3": 0.33,
        "1/2": 0.5,
        full: 1.0,
      ...
      // Get the max width for the current breakpoint
      const maxColWidth =
        {
          lg: 12,
          md: 10,
      ...
      // Calculate new width based on the percentage of max columns
      const newWidth = Math.max(
        Math.round(maxColWidth * sizeToPercent[size]),
        chartLayout.minW || 1
      );

      // Only update if width is actually changing
      const oldWidth = chartLayout.w;
      if (oldWidth === newWidth) {
        console.log(
          `No change needed - ${chartId} already at width ${newWidth} (${size}) at breakpoint ${currentBreakpoint}`
        );

        // Force a width change by temporarily setting to a different width
        // This helps react-grid-layout recognize the change
        const tempLayout = {
          ...chartLayout,
          w: newWidth + 1 > maxColWidth ? newWidth - 1 : newWidth + 1,
        };

        newCurrentLayout[chartIndex] = tempLayout;
        newLayouts[currentBreakpoint] = newCurrentLayout;

        // Update layouts to trigger a re-render with the temporary value
        setLayouts(newLayouts);

        // Then after a short delay, set it back to the desired width
        setTimeout(() => {
          const finalLayouts = { ...newLayouts };
          const finalCurrentLayout = [...finalLayouts[currentBreakpoint]];
          const finalChartIndex = finalCurrentLayout.findIndex((item) => item.i === chartId);

          finalCurrentLayout[finalChartIndex] = {
            ...finalCurrentLayout[finalChartIndex],
            w: newWidth,
          };

          finalLayouts[currentBreakpoint] = finalCurrentLayout;
        ...
        console.log(
          `Changing ${chartId} width from ${oldWidth} to ${newWidth} (${size}) at breakpoint ${currentBreakpoint}`
        );
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      setTimeout(() => {
        // Force rerender by updating a copy of the layouts
  ...
  const handleLayoutChange = (currentLayout, allLayouts) => {
    // Only update if we have complete layout information
    if (!allLayouts || Object.keys(allLayouts).length === 0) {
      return;
    }

    // Create a deep copy to ensure we're not mutating any references
    const layoutsCopy = JSON.parse(JSON.stringify(allLayouts));
  ...
  return (
    <Box sx={customStyles.dashboardContainer}>
      {maximizedChart ? (
        ...
        <>
          ...
          <ResponsiveGridLayout
            ...
            isBounded={false}
            key={`${mounted ? "mounted" : "unmounted"}-${JSON.stringify(layouts).length}`}
          >
      ...
      <Menu
        ...
        anchorOrigin={{ horizontal: "right", vertical: "bottom" }}
      >
        {/* Size options with special styling for active size */}
        {activeMenu &&
          layouts &&
          (() => {
            ...
            // Return menu items with active indicator
            return (
              <>
                <MenuItem
