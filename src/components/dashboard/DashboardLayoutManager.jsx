...
export default function DashboardLayoutManager({
  ...
  const [activeChartSettings, setActiveChartSettings] = useState(
  ...
  const toggleChartVisibility = (chartId) => {
    setActiveChartSettings((settings) =>
      settings.map((s) => (s.id === chartId ? { ...s, visible: !s.visible } : s))
    );
  };
  ...
  const setChartSize = (chartId, size) => {
    setActiveChartSettings((settings) =>
      settings.map((s) => (s.id === chartId ? { ...s, size } : s))
    );
  };
                ...
                {savedLayouts.map((layout) => (
                  <React.Fragment key={layout.id}>
                    ...
                    >
                      <ListItemText
                        primary={layout.name}
                        secondary={new Date(layout.created).toLocaleString()}
                      />
                      <ListItemSecondaryAction>
          ...
          <>
            ...
            <List sx={{ maxHeight: "60vh", overflow: "auto" }}>
              {availableCharts.map((chart, index) => {
                const chartSetting = activeChartSettings.find((s) => s.id === chart.id) || {
                  visible: true,
                  size: chart.defaultSize || "md",
                };
                  ...
                  <React.Fragment key={chart.id}>
                    <ListItem>
                      <FormControlLabel
                        control={
                          <Checkbox
                            checked={chartSetting.visible}
                            onChange={() => toggleChartVisibility(chart.id)}
                          />
                        }
                        label={
                          <ListItemText
                            primary={t(`charts.${chart.id.replace(/-/g, "")}.title`)}
                            secondary={t(`charts.${chart.id.replace(/-/g, "")}.description`)}
                          />
                        }
                      />
                      <ListItemSecondaryAction>
                        <Box sx={{ display: "flex", gap: 1 }}>
                          <Button
                            variant={chartSetting.size === "sm" ? "contained" : "outlined"}
                            size="small"
                            onClick={() => setChartSize(chart.id, "sm")}
                          >
                            S
                          </Button>
                          <Button
                            variant={chartSetting.size === "md" ? "contained" : "outlined"}
                            size="small"
                            onClick={() => setChartSize(chart.id, "md")}
                          >
                            M
                          </Button>
                          <Button
                            variant={chartSetting.size === "lg" ? "contained" : "outlined"}
                            size="small"
                            onClick={() => setChartSize(chart.id, "lg")}
                          >
                            L
                          </Button>
                        </Box>
...
DashboardLayoutManager.propTypes = {
