...
export function CountryComparisonChart({ data = [] }: CountryComparisonChartProps): JSX.Element {
  ...
  const groupedData: Record<string, CountryData> = dataArray.reduce((acc, project) => {
  ...
  const chartData: CountryData[] = Object.values(groupedData)
  ...
  const valueFormatter = (value: number | null, type: string): string => {
    if (value === null || isNaN(Number(value))) {
      return type === "capacity" ? "0 MW" : "$0M";
    }
