...
interface ChartDataPoint {
  year: string;
  [key: string]: string | number; // For dynamic metric names
  ratio: number;
  quarter: string;
  yearQuarter: string;
}
...
export function RechartsExample({ data }: RechartsExampleProps): JSX.Element {
  ...
  const chartData: ChartDataPoint[] = data.years.map((year, index) => {
  ...
  const avgEbitda =
    chartData.reduce((sum, item) => sum + (item[t("metric.ebitda")] as number), 0) /
    chartData.length;

  // Get the maximum value for domain setting with some additional padding
  const maxValue =
    Math.max(
      ...chartData.map((item) => item[t("metric.revenue")] as number),
      ...chartData.map((item) => item[t("metric.ebitda")] as number)
    ) * 1.2; // Add 20% padding

  // Custom event handlers for interactive elements
  const handleMouseEnter = (_data: any, index: number): void => {
    setActiveItem(index);
  };
  ...
  const handleLegendClick = (quarter: string): void => {
  ...
  const labelFormatter = (label: string): string => `Year: ${label}`;

  // Group years by quarters for highlighting
  const quarters: Record<string, string[]> = chartData.reduce(
  ...
  const growthPeriods: GrowthPeriod[] = chartData.reduce(
    (acc: GrowthPeriod[], item, index, arr) => {
      if (index < 2) return acc;

      const currentEbitda = item[t("metric.ebitda")] as number;
      const prevEbitda = arr[index - 1][t("metric.ebitda")] as number;
      const prevPrevEbitda = arr[index - 2][t("metric.ebitda")] as number;
  ...
  return (
    <div className="chart-container">
        ...
        <div style={{ display: "flex", gap: "4px" }}>
          {Object.keys(quarters).map((quarter, i) => (
            <button
              ...
              style={{
                background:
                  highlightedQuarter === `Q${i + 1}` ? ChartTheme.colors.accent[i] : "#f1f5f9",
      ...
      <ResponsiveContainer
        ...
        height="100%"
        aspect={2.5}
        className={animationEnabled ? "animate-chart" : ""}
      >
        <ComposedChart
          data={chartData}
          margin={{
        ...
        >
          ...
          <YAxis
            ...
            tickLine={{ stroke: "#888" }}
            axisLine={{ stroke: "#888" }}
            tickFormatter={(value: number) => `${currency.symbol}${value}M`}
