// ChartTheme.jsx - Common styling for all charts in the application
...
export const getChartColors = (type, count = 1) => {
	switch (type) {
		case "bar":
		case "area":
			return count === 1 ? ChartTheme.colors.primary[0] : ChartTheme.colors.primary.slice(0, count);
		case "line":
			return count === 1 ? ChartTheme.colors.secondary[0] : ChartTheme.colors.secondary.slice(0, count);
		case "pie":
		case "donut":
			return [...ChartTheme.colors.primary, ...ChartTheme.colors.secondary, ...ChartTheme.colors.success].slice(0, count);
		case "success":
		...
		default:
			return count === 1 ? ChartTheme.colors.primary[0] : [...ChartTheme.colors.primary, ...ChartTheme.colors.secondary].slice(0, count);
	}
};

// Helper to create gradient for area charts - avoiding direct JSX
export const createGradient = (id, color, opacity = 0.2) => {
	...
	return React.createElement("linearGradient", { id, x1: "0", y1: "0", x2: "0", y2: "1" }, [
		...
		React.createElement("stop", { key: "stop2", offset: "95%", stopColor: color, stopOpacity: opacity })
	]);
};

// Generate dashed patterns for chart elements
export const getDashPattern = index => {
