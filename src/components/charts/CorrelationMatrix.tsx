...
export const CorrelationMatrix: React.FC<CorrelationMatrixProps> = ({ data }) => {
  ...
  const [metricGroup, setMetricGroup] = useState<string>("financial");
  ...
  const projectData: Project[] = data?.projects || sampleData.projects;
  ...
  const calculateCorrelationMatrix = (data: Project[], metrics: Metric[]): number[][] => {
    const matrix: number[][] = [];

    // Extract metric values from data
    const metricValues: Record<string, number[]> = {};
    metrics.forEach((metric) => {
      metricValues[metric.id] = data.map((item) => Number(item[metric.id]) || 0);
    });
      ...
      metrics.forEach((metric2, j) => {
        if (i === j) {
          ...
          const correlation = calculateCorrelation(
            metricValues[metric1.id],
            metricValues[metric2.id]
          );
  ...
  useEffect(() => {
    const renderCorrelationMatrix = async (): Promise<void> => {
      try {
        setLoading(true);
        ...
        const cells = rows
          .selectAll<SVGRectElement, CellData>(".cell")
          ...
          .attr("y", 0)
          .attr("width", Math.max(1, cellSize))
          .attr("height", Math.max(1, cellSize))
        ...
        rows
          .selectAll<SVGTextElement, CellData>(".cell-text")
          .data((d, i) => d.map((value, j) => ({ value, i, j })))
          .enter()
          .append("text")
  ...
  }, [dimensions, metricGroup, projectData, theme, metricGroups]);
  ...
  const correlationType = (value: number): string => {
  ...
  const handleMetricGroupChange = (e: SelectChangeEvent<string>): void => {
    setMetricGroup(e.target.value);
  };
    ...
    >
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
        ...
        <FormControl variant="outlined" size="small">
          ...
          <Select
            labelId="metric-group-label"
            value={metricGroup}
            onChange={handleMetricGroupChange}
            label={t("charts.correlation.metricGroup")}
            sx={{ minWidth: 150 }}
          >
      ...
      {hoveredCell && (
        ...
        >
          ...
          <Typography variant="body2" sx={{ mt: 0.5 }}>
            {correlationType(hoveredCell.correlation)} {t("charts.correlation.correlation")} (
            {hoveredCell.correlation.toFixed(2)})
          </Typography>
