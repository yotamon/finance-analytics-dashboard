 ...
 */
export const ThemeProvider = ({ children, defaultTheme = "light", externalTheme = null }) => {
  // Initialize theme state from external theme, localStorage or default
  const [theme, setTheme] = useState(() => {
    // If an external theme is provided, use it
    if (externalTheme) {
      return externalTheme;
    }
    // Try to get theme from localStorage
    const savedTheme =
      typeof window !== "undefined" ? localStorage.getItem("finance-analyzer-theme") : null;
    return savedTheme || defaultTheme;
  });

  // Update theme if external theme changes
  useEffect(() => {
    if (externalTheme) {
