...
import { Save, RefreshCw, Check, X, AlertCircle, Globe, DollarSign, Palette, RotateCcw } from "lucide-react";
...
const Settings = () => {
	...
	const handleLocaleChange = (setting, value) => {
	...
	const handleResetColors = () => {
		if (window.confirm("Are you sure you want to reset all colors to default? This cannot be undone.")) {
			resetThemeColors();
			...
			<div className="max-w-4xl mx-auto">
				...
				{testResult && (
					<div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md text-blue-800">
						...
						<div className="flex items-center">
							...
							<span>
								Setting <code className="bg-blue-100 px-1 rounded">{testResult.setting}</code> has value: <code className="bg-blue-100 px-1 rounded">{JSON.stringify(testResult.value)}</code>
							</span>
						</div>
					</div>
				)}
				...
				<div className="space-y-6">
					...
					<div className="bg-[rgb(var(--bg-tertiary))] rounded-lg shadow-sm border border-[rgb(var(--border-light))] p-6">
						<h2 className="text-xl font-semibold mb-4 text-[rgb(var(--text-primary))]">Localization</h2>
					...
					<div className="bg-[rgb(var(--bg-tertiary))] rounded-lg shadow-sm border border-[rgb(var(--border-light))] p-6">
						<h2 className="text-xl font-semibold mb-4 text-[rgb(var(--text-primary))]">Company Details</h2>
						<div className="space-y-4">
							<div>
								<label htmlFor="companyName" className="block text-sm font-medium text-[rgb(var(--text-primary))] mb-1">
									Company Name
								</label>
								<input
									type="text"
							...
							<div>
								<label htmlFor="tagline" className="block text-sm font-medium text-[rgb(var(--text-primary))] mb-1">
									Tagline
								</label>
								<input
									type="text"
							...
							<div>
								<label htmlFor="description" className="block text-sm font-medium text-[rgb(var(--text-primary))] mb-1">
									Description
								</label>
								<textarea
									id="description"
							...
							<div>
								<label htmlFor="contactEmail" className="block text-sm font-medium text-[rgb(var(--text-primary))] mb-1">
									Contact Email
								</label>
								<input
									type="email"
							...
							<div>
								<label htmlFor="website" className="block text-sm font-medium text-[rgb(var(--text-primary))] mb-1">
									Website
								</label>
								<input
									type="url"
						...
						<div className="mt-4 pt-4 border-t border-[rgb(var(--border-light))]">
							<p className="text-sm text-[rgb(var(--text-secondary))]">These settings affect how your company information appears throughout the application.</p>
						</div>
					</div>

					{/* Feature Flags Section */}
					<div className="bg-[rgb(var(--bg-tertiary))] rounded-lg shadow-sm border border-[rgb(var(--border-light))] p-6">
						<h2 className="text-xl font-semibold mb-4 text-[rgb(var(--text-primary))]">Feature Flags</h2>
						<div className="space-y-4">
							{Object.entries(featureFlags).map(([feature, enabled]) => (
								<div key={feature} className="flex items-center justify-between">
									<div className="max-w-[80%]">
										...
										<p className="text-xs text-[rgb(var(--text-secondary))] mt-1">{getFeatureDescription(feature)}</p>
									</div>
									<label className="inline-flex items-center cursor-pointer">
										<input type="checkbox" className="sr-only peer" checked={enabled} onChange={() => handleFeatureToggle(feature)} />
						...
						<div className="mt-4 pt-4 border-t border-[rgb(var(--border-light))]">
							<p className="text-sm text-[rgb(var(--text-secondary))]">
								Feature flags control which features are available in the application. Changes to feature flags may require refreshing the page to take effect.
							</p>
						</div>
					</div>
					...
					<div className="bg-[rgb(var(--bg-tertiary))] rounded-lg shadow-sm border border-[rgb(var(--border-light))] p-6">
						...
						<p className="text-sm text-[rgb(var(--text-secondary))] mb-4">Customize the application colors to match your brand or preferences. Changes will be applied when you save settings.</p>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							<div className="space-y-4">
								...
								<div>
									<label className="flex items-center justify-between text-sm font-medium text-[rgb(var(--text-primary))] mb-2">
										Primary Color
										<div className="w-5 h-5 rounded-full border border-[rgb(var(--border-light))]" style={{ backgroundColor: colorSettings.primary }}></div>
									</label>
									<div className="flex">
										<input
											type="color"
								...
								<div>
									<label className="flex items-center justify-between text-sm font-medium text-[rgb(var(--text-primary))] mb-2">
										Secondary Color
										<div className="w-5 h-5 rounded-full border border-[rgb(var(--border-light))]" style={{ backgroundColor: colorSettings.secondary }}></div>
									</label>
									<div className="flex">
										<input
											type="color"
							...
							<div className="space-y-4">
								...
								<div>
									<label className="flex items-center justify-between text-sm font-medium text-[rgb(var(--text-primary))] mb-2">
										Success Color
										<div className="w-5 h-5 rounded-full border border-[rgb(var(--border-light))]" style={{ backgroundColor: colorSettings.success }}></div>
									</label>
									<div className="flex">
										<input
											type="color"
								...
								<div>
									<label className="flex items-center justify-between text-sm font-medium text-[rgb(var(--text-primary))] mb-2">
										Warning Color
										<div className="w-5 h-5 rounded-full border border-[rgb(var(--border-light))]" style={{ backgroundColor: colorSettings.warning }}></div>
									</label>
									<div className="flex">
										<input
											type="color"
								...
								<div>
									<label className="flex items-center justify-between text-sm font-medium text-[rgb(var(--text-primary))] mb-2">
										Error Color
										<div className="w-5 h-5 rounded-full border border-[rgb(var(--border-light))]" style={{ backgroundColor: colorSettings.error }}></div>
									</label>
									<div className="flex">
										<input
											type="color"
						...
						<div className="mt-4 pt-4 border-t border-[rgb(var(--border-light))]">
							<div className="flex items-center justify-between">
								<p className="text-sm text-[rgb(var(--text-secondary))]">Preview your theme colors before saving. The color changes will be applied to the entire application.</p>
					...
					<div className="bg-[rgb(var(--bg-tertiary))] rounded-lg shadow-sm border border-[rgb(var(--border-light))] p-6">
						<h2 className="text-xl font-semibold mb-4 text-[rgb(var(--text-primary))]">Configuration Information</h2>
						<div className="space-y-4">
							<p className="text-[rgb(var(--text-primary))]">Settings are stored in your browser's localStorage and will persist across page reloads and browser sessions.</p>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div className="bg-[rgb(var(--bg-secondary))] p-4 rounded border border-[rgb(var(--border-light))]">
									...
									<div className="text-sm text-[rgb(var(--text-secondary))]">
										...
										<p>
											<span className="font-medium">Persistence:</span> Until manually cleared or reset
										</p>
									</div>
								</div>
